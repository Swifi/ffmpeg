...
#define ASIF                    0
#define ASIF_C_VERSION1         0xA2805140

typedef struct ASIFInputContext {
    int64_t data_end;
    int block_duration;
} ASIFInputContext;

static enum AVCodecID asif_codec_get_id(int bps)
{
    if (bps <= 8)
        return AV_CODEC_ID_ASIF;
...
static int get_asif_header(AVFormatContext *s, int size,
                                    unsigned version)
{
    AVIOContext *pb        = s->pb;
    AVCodecParameters *par = s->streams[0]->codecpar;
    ASIFInputContext *asif = s->priv_data;
    int exp;
    uint64_t val;
    int sample_rate;
    ...
    /* get codec id for ASIF-C */
    if (size < 4) {
        version = ASIF;
    } else if (version == ASIF_C_VERSION1) {
        par->codec_tag = avio_rl32(pb);
        par->codec_id  = ff_codec_get_id(ff_codec_asif_tags, par->codec_tag);
    ...
    if (version != ASIF_C_VERSION1 || par->codec_id == AV_CODEC_ID_PCM_S16BE) {
        par->codec_id = asif_codec_get_id(par->bits_per_coded_sample);
        par->bits_per_coded_sample = av_get_bits_per_sample(par->codec_id);
        asif->block_duration = 1;
    } else {
        switch (par->codec_id) {
        ...
        if (par->block_align > 0)
            asif->block_duration = av_get_audio_frame_duration2(par,
                                                                par->block_align);
    }
...
static int asif_read_header(AVFormatContext *s)
{
    ...
    uint32_t tag;
    unsigned version = ASIF_C_VERSION1;
    AVIOContext *pb = s->pb;
    AVStream * st;
    ASIFInputContext *asif = s->priv_data;
    ...
    /* ASIF data type */
    tag = avio_rl32(pb);
    if (tag == MKTAG('A', 'I', 'F', 'F'))       /* Got an ASIF file */
        version = ASIF;
    else if (tag != MKTAG('A', 'I', 'F', 'C'))  /* An ASIF-C file then */
        return AVERROR_INVALIDDATA;
    ...
    while (filesize > 0) {
        ...
        case MKTAG('C', 'O', 'M', 'M'):     /* Common chunk */
            ...
            st->nb_frames = get_asif_header(s, size, version);
            if (st->nb_frames < 0)
                return st->nb_frames;
        ...
        case MKTAG('S', 'S', 'N', 'D'):     /* Sampled sound chunk */
            asif->data_end = avio_tell(pb) + size;
        ...
        case MKTAG('w', 'a', 'v', 'e'):
            ...
            if (   (st->codecpar->codec_id == AV_CODEC_ID_QDMC || st->codecpar->codec_id == AV_CODEC_ID_QDM2)
                ...
                asif->block_duration = AV_RB32(st->codecpar->extradata+9*4);
...
got_sound:
    ...
    st->start_time = 0;
    st->duration = st->nb_frames * asif->block_duration;
...
static int asif_read_packet(AVFormatContext *s,
                            AVPacket *pkt)
{
    ...
    max_size = asif->data_end - avio_tell(s->pb);
    if (max_size <= 0)
        return AVERROR_EOF;
    ...
    /* Only one stream in an ASIF file */
    pkt->stream_index = 0;
    pkt->duration     = (res / st->codecpar->block_align) * asif->block_duration;
    return 0;
}
...
AVInputFormat ff_asif_demuxer = {
    ...
    .priv_data_size = sizeof(ASIFInputContext),
